...
namespace tcproute
{
    ...
    /// @@FROM http://stackoverflow.com/questions/10360933/find-interfaces-and-ip-addresses-arp-in-c
    /// </summary>
    public class IPInfo
    {
        ...
        /// <param name="macAddress">The MAC Address of the IPInfo to retrieve.</param>
        /// <returns></returns>
        public static IPInfo GetMACInfo(string ipaddress)
        {
            //Console.WriteLine("@@looking up " + ipaddress);
            var ipinfo = (from ip in IPInfo.GetIPInfo()
                          where ip.IPAddress.ToLowerInvariant() == ipaddress.ToLowerInvariant()
                          select ip).FirstOrDefault();
